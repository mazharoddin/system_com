...
class ComplaintsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create complaint" do
    assert_difference('Complaint.count') do
      post complaints_url, params: { complaint: { created: @complaint.created, date: @complaint.date, name: @complaint.name, section: @complaint.section, status: @complaint.status, user_id: @complaint.user_id } }
    end
  ...
  test "should update complaint" do
    patch complaint_url(@complaint), params: { complaint: { created: @complaint.created, date: @complaint.date, name: @complaint.name, section: @complaint.section, status: @complaint.status, user_id: @complaint.user_id } }
